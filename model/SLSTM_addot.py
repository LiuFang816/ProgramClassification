...
class StackLSTM(object):
    ...
    # different than reported in the paper.
        ...
        def func_pop(time_step,batch,state,STATES):
            ...
            STATES[batch]=self.state_stack.pop()
            NEW_STATES=tf.concat(STATES,1)
            ...
            # print([tf.reshape(state[0][0][batch],[1,-1]),tf.reshape(state[0][1][batch],[1,-1]),tf.reshape(state[1][0][batch],[1,-1]),tf.reshape(state[1][1][batch],[1,-1])])
            return [tf.reshape(state[0][0][batch],[1,-1]),tf.reshape(state[0][1][batch],[1,-1]),tf.reshape(state[1][0][batch],[1,-1]),tf.reshape(state[1][1][batch],[1,-1])]
        #def func_pop(state):
        #    #------增加一层---------------#
        #    w = tf.get_variable(
            ...
            for time_step in range(self.config.num_steps):
                ...
                for i in range(config.batch_size):
                    new_state_i=tf.cond(tf.equal(self.input[i][time_step],self.start),
                                             lambda:func_push(STATES[i]),lambda:func_default(STATES[i]))
                    new_state_i=tf.cond(tf.equal(self.input[i][time_step],self.end),
                                      lambda:func_pop(time_step,i,state,STATES),lambda:func_default(STATES[i]))
                    state_i=new_state_i
